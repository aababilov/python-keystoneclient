...
'            requests.Session.request (such as `headers`) or `json`
    ...
    def _get_token_and_endpoint(self, client, url_for_args, silent):
...
'            `HTTPClient.request`
        ...
        token, endpoint = self._get_token_and_endpoint(
            client, url_for_args, silent=True)
        just_authenticated = False
        if not (endpoint and token):
            self.authenticate()
            just_authenticated = True
            token, endpoint = self._get_token_and_endpoint(
                client, url_for_args, silent=False)
            if not (endpoint and token):
